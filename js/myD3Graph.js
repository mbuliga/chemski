...
//
// https://github.com/mbuliga/quinegraphs/blob/master/js/myD3Graph.js
//
// last updated: 14.08.2020
// added tag and number fields to nodes and edges, to be used for global fanout GLOBALFO
// added token field to nodes, to be used with tokens
...
// added tag and number fields, see for the global parameters tagIni and numberIni parameters.js
// added token field for the introduction of tokens
  this.addNode = function (id, type, x, y) {
    nodes.push({"id": id, "type": type, x: x, y: y, vx:0, vy:0, links:[], "age":age, tag:tagIni, number:numberIni, token:tokenIni});
    return nodes[nodes.length-1];
  };
  this.addNodeWithDeco = function (id, type, x, y, deco) {
    nodes.push({"id": id, "type": type, x: x, y: y, vx:0, vy:0, links:[], "age":age, tag:deco, number:numberIni, token:tokenIni});
    return nodes[nodes.length-1];
  };
...
added tag and number
*/
  this.addLink = function (source, target, value) {
    var nsource = findNode(source);
    var ntarget = findNode(target);
    var newLink = {"source": nsource, "target": ntarget, "value": value, "age":age, tag:tagIni, number:numberIni};
...
// adds a center node and ports of a given type, see in nodes.js how a graph is obtained from a mol node
function addNodeAndEdgesWithDeco(type,x,y,deco) {
  x = x || w/2;
  y = y || h/2;
  ...
  if (!valence) throw new TypeError("Unknown type" +  type);
  if (deco.length != valence) throw new TypeError("Unknown decoration" +  type);

  var portTypes = nodePortTypes[valence.length-1];
  var portList = [i];
  
  for (k=0; k<valence.length; k++) {
    addNodeWithDeco(i+k+1, portTypes[k], x + (Math.random()*20), y + (Math.random()*20),deco[k])
    findNode(i+k+1).dir = valence[k];
    portList.push(i+k+1);
  }

...
//
// modification for decorations with lambda terms of the edges, for the GLOBALFO of COMBINALAMBDA
//
...
// adds to the node which is not center the half-edge number from the mol file
            currNode = findNode(other.id);
            currNode.number = edgeCount;
            currNode = findNode(linked[k].id);
            currNode.number = edgeCount;
//
            line += " " + edgeCount;
          } else {
...
//          
      ...
      line += addedFR;
      result += line;
      nodes[i].number = numberOfCenterNodes;
...
// updates the d3 db
  update();
//  var addToResult = "";
//  addToResult += numberOfCenterNodes + "<br>";
//  for (var i=0; i<nodes.length; i++) {
//    addToResult += i + " " + nodes[i].number + "<br>";
//  }
//  document.getElementById("nodenumber").innerHTML = addToResult;
...
function takeMolFromScreen() {  

  var result =  document.getElementById("molexportafter").innerHTML;
  if (result == "") { result =  document.getElementById("molexport").innerHTML;}
  return result;

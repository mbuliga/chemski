// initially contained parser for lambda calculus to mol and decorator, to be adapted to chemSKI or COMBINALAMBDA, to be a parser for SKI too
// 
...
// transform it into an array 
  var molFinalArray = [], molLine = [], linkedi = [], frinSuccess;
...
// end decorator for nodes S, K, I
      ...
      if (frinSuccess == 1) {
        if (linkedi[0].tag != "") {
          molFinalEdgesInDeco[molFinalArray[i][1]] = "(" + linkedi[0].tag + ")";
        } else {
...
// main decoration loop
  ...
  var resultDeco = {compatible:nodesCompatibility, molfinaldeco:molFinalDeco, molfinalarray:molFinalArray, molfinaledgesoutdeco:molFinalEdgesOutDeco, equalrel:equalRel};

  return resultDeco;
  } else {
  var resultDeco = {compatible:nodesCompatibility, molfinaldeco:[], molfinalarray:[], molfinaledgesoutdeco:[], equalrel:[]};
  return resultDeco;
  }
}

function decoratorLambdaEdges(){

  var tagPort;
  
  var molString =  takeMolFromScreen();
  var inputFromDeco = decoratorLambda0(molString,globCount);
  var molFinalEdgesOutDeco = inputFromDeco.molfinaledgesoutdeco;

  var isCompatible = inputFromDeco.compatible;

  if (isCompatible == 0) {
  for (var i=0; i<nodes.length; i++) {
    if (!isCenter(nodes[i])) { 
      tagPort = nodes[i].number;
      nodes[i].tag = molFinalEdgesOutDeco[tagPort];
    }
...
function decoratorLambda(){
  ...
  var inputFromDeco = decoratorLambda0(molString,globalPref);
